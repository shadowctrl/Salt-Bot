...
const premiumCommand: SlashCommand = {
	...
	data: new discord.SlashCommandBuilder()
		...
		.addSubcommand((subcommand) => subcommand.setName('status').setDescription('Check your premium status'))
		.addSubcommand((subcommand) =>
			subcommand
				.setName('redeem')
				.setDescription('Redeem a premium coupon code')
				.addStringOption((option) => option.setName('code').setDescription('The premium coupon code').setRequired(true))
		)
		.addSubcommand((subcommand) =>
			subcommand
				.setName('remove')
				.setDescription('Remove your premium status (owner only)')
				.addUserOption((option) => option.setName('user').setDescription('The user to remove premium from').setRequired(true))
		),
	execute: async (interaction: discord.ChatInputCommandInteraction, client: discord.Client) => {
		...
		try {
			...
			switch (subcommand) {
				...
				case 'remove': {
					if (!client.config.bot.owners.includes(interaction.user.id)) return interaction.editReply({ embeds: [new EmbedTemplate(client).error('❌ This subcommand is restricted to bot owners only.')] });
					const targetUser = interaction.options.getUser('user', true);
					const [isPremium, _] = await premiumHandler.checkPremiumStatus(targetUser.id);
					if (!isPremium) return interaction.editReply({ embeds: [new EmbedTemplate(client).warning(`⚠️ ${targetUser.tag} doesn't have premium status.`)] });
					const success = await premiumHandler.revokePremium(targetUser.id);
					if (success) {
						const embed = new discord.EmbedBuilder().setTitle('✅ Premium Removed').setDescription(`Successfully removed premium status from **${targetUser.tag}**.`).setColor(client.config.embed.color.success).setTimestamp();
						await interaction.editReply({ embeds: [embed] });
						client.logger.info(`[PREMIUM] ${interaction.user.tag} (${interaction.user.id}) removed premium from ${targetUser.tag} (${targetUser.id})`);
					} else {
						await interaction.editReply({ embeds: [new EmbedTemplate(client).error(`❌ Failed to remove premium status from ${targetUser.tag}. Database operation failed.`)] });
					}
					break;
				}

				default:
					await interaction.editReply({ embeds: [new EmbedTemplate(client).error('❌ Invalid subcommand')] });
			}
		} catch (error) {
			client.logger.error(`[PREMIUM] Error executing premium command: ${error}`);
			await interaction.editReply({ embeds: [new EmbedTemplate(client).error('❌ An error occurred while processing your request.')] });
		}
	},
};
