...
export const infoTicket = async (interaction: discord.ChatInputCommandInteraction, client: discord.Client): Promise<void> => {
	...
	try {
		...
		if (ticket.claimedById && claimer) {
			embed.addFields({ name: 'Claimed By', value: `${claimer} (${claimer.tag})`, inline: true }, { name: 'Claimed At', value: claimedTimestamp ? `<t:${claimedTimestamp}:F>` : 'Unknown', inline: true });
		}

		if (ticket.status !== 'open' && closer) {
			embed.addFields({ name: 'Closed By', value: `${closer} (${closer.tag})`, inline: true }, { name: 'Closed At', value: closedTimestamp ? `<t:${closedTimestamp}:F>` : 'Unknown', inline: true }, { name: 'Reason', value: ticket.closeReason || 'No reason provided', inline: false });
		}
