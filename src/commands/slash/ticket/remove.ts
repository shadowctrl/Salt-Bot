...
export const removeUserFromTicket = async (interaction: discord.ChatInputCommandInteraction, client: discord.Client): Promise<discord.Message<boolean> | void> => {
	await interaction.deferReply();

	try {
		const ticketManager = new Ticket((client as any).dataSource, client);
		const userToRemove = interaction.options.getUser('user');
		if (!userToRemove) return await interaction.editReply({ embeds: [new EmbedTemplate(client).error('Please specify a valid user to remove.')] });
		...
		await interaction.editReply({ embeds: [new EmbedTemplate(client).error('An error occurred while removing the user from the ticket.')] });
	}
};
