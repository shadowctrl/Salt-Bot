...
const stopCommand: SlashCommand = {
	...
	data: new discord.SlashCommandBuilder()
		...
		.addSubcommand((subcommand) => subcommand.setName('disable').setDescription('Disable the ticket system entirely'))
		.addSubcommand((subcommand) => subcommand.setName('remove_panel').setDescription('Remove the ticket panel message'))
		.addSubcommand((subcommand) => subcommand.setName('close_all').setDescription('Close all open tickets')),
							...
							return interaction.editReply({
								embeds: [
									new discord.EmbedBuilder()
										...
										.setDescription(`Successfully closed ${closedCount} tickets.\n` + (failedCount > 0 ? `Failed to close ${failedCount} tickets.` : ''))
