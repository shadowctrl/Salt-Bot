...
export const deployTicketSystem = async (interaction: discord.MessageComponentInteraction, client: discord.Client, ticketRepo: TicketRepository, ticketChannel: discord.TextChannel): Promise<void> => {
	try {
		await interaction.editReply({ embeds: [new discord.EmbedBuilder().setTitle('⏳ Deploying Ticket System').setDescription('Please wait while we deploy your ticket system...').setColor('Blue')], components: [] }).catch(() => client.logger.debug('[SETUP] Failed to update loading state - continuing with deployment'));
		await wait(2000);

		const buttonConfig = await ticketRepo.getTicketButtonConfig(interaction.guildId!);
		if (!buttonConfig) throw new Error('Button configuration not found');
		...
		const panelMessage = await ticketChannel.send({ embeds: [ticketEmbed], components: [buttonRow] });
		await ticketRepo.configureTicketButton(interaction.guildId!, { messageId: panelMessage.id });
		...
		try {
			await interaction.editReply({ embeds: [new discord.EmbedBuilder().setTitle('🎉 Ticket System Deployed').setDescription(`Your ticket system has been deployed in ${ticketChannel}!\n\nUsers can now create tickets by clicking the button.`).setColor('Green').addFields({ name: 'Customization', value: 'You can customize your ticket system further with `/ticket config`' }).setTimestamp()], components: [] });
		} catch (editError) {
			client.logger.warn(`[SETUP] Failed to edit reply after deployment: ${editError}`);
			try {
				await interaction.message.reply({ embeds: [new EmbedTemplate(client).success('🎉 Ticket System Deployed!').setDescription(`The ticket panel has been created in ${ticketChannel}.`)] });
			} catch (replyError) {
		...
		try {
			await interaction.editReply({ embeds: [new EmbedTemplate(client).error('An error occurred while deploying the ticket system.')], components: [] });
		} catch (editError) {
			client.logger.debug(`[SETUP] Failed to edit reply with error: ${editError}`);
			try {
				await interaction.message.reply({ embeds: [new EmbedTemplate(client).error('An error occurred while deploying the ticket system.')] });
			} catch (replyError) {
