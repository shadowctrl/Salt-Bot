...
export const handleClearHistory = async (interaction: discord.ChatInputCommandInteraction, client: discord.Client): Promise<void> => {
	try {
		...
		if (!config || !config.enabled) {
			await interaction.editReply({
				embeds: [new EmbedTemplate(client).error('This command can only be used in a chatbot channel.')],
			});
			return;
		}
		...
		if (success) {
			await interaction.editReply({
				embeds: [new EmbedTemplate(client).success('Your chat history has been cleared!').setDescription(`${config.chatbotName} will no longer remember your previous conversations.`)],
			});
		} else {
			await interaction.editReply({
				embeds: [new EmbedTemplate(client).error('Failed to clear your chat history.').setDescription('Please try again later or contact an administrator.')],
			});
		}
