...
export const handleDelete = async (interaction: discord.ChatInputCommandInteraction, client: discord.Client, chatbotRepo: ChatbotConfigRepository): Promise<void> => {
	try {
		const existingConfig = await chatbotRepo.getConfig(interaction.guildId!);
		if (!existingConfig) {
			await interaction.editReply({
				embeds: [new EmbedTemplate(client).error('No chatbot is set up for this server.')],
			});
			return;
		}
		...
		try {
			...
			if (response === 'confirm') {
				...
				try {
					...
					if (botChannel && botChannel.isTextBased()) {
						await botChannel.send({
							embeds: [new discord.EmbedBuilder().setTitle('Chatbot Deactivated').setDescription('This channel is no longer configured as a chatbot channel.').setColor('Red').setTimestamp()],
						});
					}
				} catch (error) {
				...
				await interaction.editReply({
					embeds: [new EmbedTemplate(client).success('Chatbot configuration deleted successfully!').setDescription('The chatbot has been deactivated and all settings have been removed.')],
				});
				return;
			} else {
				await interaction.editReply({
