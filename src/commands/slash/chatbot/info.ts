...
export const handleInfo = async (interaction: discord.ChatInputCommandInteraction, client: discord.Client, chatbotRepo: ChatbotConfigRepository): Promise<discord.Message<boolean> | void> => {
	try {
		...
		try {
			...
			if (channel) channelInfo = `<#${channel.id}>`;
		} catch (error) {
		...
		if (existingConfig.responseType) infoEmbed.addFields({ name: 'Response Type', value: existingConfig.responseType.length > 1024 ? existingConfig.responseType.substring(0, 1021) + '...' : existingConfig.responseType, inline: false });
		await interaction.editReply({ embeds: [infoEmbed] });
		return;
	} catch (error) {
		client.logger.error(`[CHATBOT_INFO] Error getting chatbot info: ${error}`);
		await interaction.editReply({ embeds: [new EmbedTemplate(client).error('An error occurred while getting chatbot information.')] });
	}
};
