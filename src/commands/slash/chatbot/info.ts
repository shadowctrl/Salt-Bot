...
export const handleInfo = async (interaction: discord.ChatInputCommandInteraction, client: discord.Client, chatbotRepo: ChatbotConfigRepository): Promise<void> => {
	try {
		...
		if (existingConfig.responseType) {
			infoEmbed.addFields({
				name: 'Response Type',
				value: existingConfig.responseType.length > 1024 ? existingConfig.responseType.substring(0, 1021) + '...' : existingConfig.responseType,
				inline: false,
			});
		}
