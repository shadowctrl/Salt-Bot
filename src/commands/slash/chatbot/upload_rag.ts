		...
		await interaction.editReply({
			embeds: [
				new EmbedTemplate(client)
					.success('Knowledge Data Added Successfully!')
					.setDescription(`Your knowledge base has been processed and added to the chatbot.`)
					.addFields({ name: 'File', value: file.name, inline: true }, { name: 'Chunks Created', value: storedDocument.chunkCount.toString(), inline: true }, { name: 'Size', value: `${Math.round(file.size / 1024)} KB`, inline: true }),
			],
		});
