...
export const handleSettings = async (interaction: discord.ChatInputCommandInteraction, client: discord.Client, chatbotRepo: ChatbotConfigRepository): Promise<void> => {
	try {
		const existingConfig = await chatbotRepo.getConfig(interaction.guildId!);
		if (!existingConfig) {
			await interaction.editReply({
				embeds: [new EmbedTemplate(client).error('No chatbot is set up for this server.').setDescription('Use `/chatbot setup` to create a new chatbot configuration.')],
			});
			return;
		}
		...
		if (apiKey || modelName || baseUrl) {
			try {
				const llm = new LLM(apiKey || existingConfig.apiKey, baseUrl || existingConfig.baseUrl);
				await llm.invoke([{ role: 'user', content: "Say 'API connection successful'" }], modelName || existingConfig.modelName, { max_tokens: 50 });
			} catch (error) {
				await interaction.editReply({
					embeds: [new EmbedTemplate(client).error('Failed to connect to the API with the new settings.').setDescription(`Error: ${error instanceof Error ? error.message : String(error)}`)],
				});
				return;
			}
		}
		...
		await interaction.editReply({
			embeds: [
				new EmbedTemplate(client)
					...
					.addFields({ name: 'Name', value: updatedConfig.chatbotName, inline: true }, { name: 'Model', value: updatedConfig.modelName, inline: true }, { name: 'Cooldown', value: `${updatedConfig.cooldown} seconds`, inline: true }, { name: 'API', value: updatedConfig.baseUrl, inline: true }),
			],
		});
		return;
	} catch (error) {
