...
export class EncryptionUtil {
	...
	public static isEncrypted = (data: string): boolean => {
		...
		try {
			...
			return salt.length === EncryptionUtil.SALT_LENGTH && iv.length === EncryptionUtil.IV_LENGTH && tag.length === EncryptionUtil.TAG_LENGTH;
		} catch {
			return false;
		}
	};
		...
		try {
			...
			if (!masterKey) {
				return {
					...
					recommendations: ['Set MASTER_ENCRYPTION_KEY environment variable', 'Use a key of at least 32 characters', 'Include uppercase, lowercase, numbers, and special characters', 'Never commit the key to version control'],
				};
			}
