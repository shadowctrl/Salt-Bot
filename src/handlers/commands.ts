 ...
 */
const loadCommandsRecursive = async (client: discord.Client, directory: string, fileFilter: (file: string) => boolean): Promise<(Command | SlashCommand)[]> => {
...
const event: BotEvent = {
	...
	execute: async (client: discord.Client): Promise<void> => {
		...
		if (!client.config.bot.command.disable_message) {
			...
			const messageCommands = (await loadCommandsRecursive(client, messageCommandsDir, (file) => file.endsWith('.js') || file.endsWith('.ts'))) as Command[];
		...
		const loadedSlashCommands = (await loadCommandsRecursive(client, slashCommandsDir, (file) => (file.endsWith('.js') || file.endsWith('.ts')) && !file.includes('.d.ts'))) as SlashCommand[];

		loadedSlashCommands.forEach((command) => {
			const shouldRegister = !client.config.bot.command.register_specific_commands.enabled || client.config.bot.command.register_specific_commands.commands.includes(command.data.name);
