	...
	public sendWelcomeMessage = async (channel: discord.TextChannel, ticket: ITicket, category: ITicketCategory, userId: string): Promise<void> => {
		try {
			...
			const welcomeEmbed = new discord.EmbedBuilder()
				...
				.addFields({ name: 'Ticket ID', value: `#${ticket.ticketNumber}`, inline: true }, { name: 'Category', value: `${category.emoji || '🎫'} ${category.name}`, inline: true }, { name: 'Status', value: `🟢 Open`, inline: true }, { name: 'Created By', value: `<@${userId}>`, inline: true }, { name: 'Created At', value: `<t:${creationTimestamp}:F>`, inline: true })
				.setColor('Green')
	...
	public sendCloseMessage = async (channel: discord.TextChannel, ticket: ITicket, userId: string, reason?: string): Promise<void> => {
		try {
			...
			const closeEmbed = new discord.EmbedBuilder()
				...
				.addFields({ name: 'Ticket ID', value: `#${ticket.ticketNumber}`, inline: true }, { name: 'Category', value: `${category.emoji || '🎫'} ${category.name}`, inline: true }, { name: 'Status', value: `🔴 Closed`, inline: true }, { name: 'Closed By', value: `<@${userId}>`, inline: true }, { name: 'Closed At', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true }, { name: 'Reason', value: reason || 'No reason provided', inline: false })
				.setColor('Red')
