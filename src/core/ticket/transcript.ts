...
export class TicketTranscript {
	...
	public createTranscript = async (channel: discord.TextChannel, ticketNumber: number): Promise<discord.AttachmentBuilder | null> => {
		try {
			const attachment = await createTranscript(channel, { limit: 10000, saveImages: true, poweredBy: false, filename: `ticket-${ticketNumber}.html` });
	 ...
	 */
	public sendTranscriptToUser = async (user: discord.User, attachment: discord.AttachmentBuilder, ticketInfo: { ticketNumber: number; guildName: string; categoryName: string; generatedBy: string }): Promise<boolean> => {
		try {
	 ...
	 * @returns Promise resolving to success status
	 */
	public sendTranscriptToChannel = async (channel: discord.TextChannel, attachment: discord.AttachmentBuilder, ticketInfo: { ticketNumber: number; creatorId: string; categoryName: string; status: string; createdAt: Date; closedBy?: string; closeReason?: string }): Promise<boolean> => {
		try {
			const creator = await client.users.fetch(ticketInfo.creatorId).catch(() => null);
			let closedByUser = null;
			if (ticketInfo.closedBy) closedByUser = await client.users.fetch(ticketInfo.closedBy).catch(() => null);
			const embed = new discord.EmbedBuilder()
				.setTitle(`Ticket #${ticketInfo.ticketNumber} Transcript`)
				...
				.setColor('#2F3136')
				.setFooter({ text: 'Automated transcript generation', iconURL: client.user?.displayAvatarURL() })
				.setTimestamp();
			if (ticketInfo.closedBy && closedByUser) embed.addFields({ name: 'Closed By', value: closedByUser.tag, inline: true }, { name: 'Close Reason', value: ticketInfo.closeReason || 'No reason provided', inline: true });
